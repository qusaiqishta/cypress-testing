describe('Intercept Mock Api', () => {
    ...
    it('mocked a ready todo', () => {
        ...
        cy.intercept('GET', '**/superheroes.json', newObj)
        cy.visit('/heroes-finished.html')
...
describe('Verify Number Of Views Of Specific Article',()=>{
    it('Should be visible when it less than 500',()=>{
        ...
        // cy.get('.pageviews').should('to.be.visible')
    ...
    it('should number of views comes from api equal to one exist in front end page',()=>{
        ...
        .then(response => {
            ...
            // viewsFromApi=response.body['views']
            // cy.get('.pageviews').then(ele=>{
            //     const viewsFromPage=Number(ele.text())
            //     expect(Number(viewsFromApi)).to.equal(viewsFromPage)
            })
        })
